...
DIALECT = 'dialect15524808879199223808'
...
# RELIABILITY
enums['RELIABILITY'] = {}
RELIABILITY_FAILED = 0 # Data is reliabile.
enums['RELIABILITY'][0] = EnumEntry('RELIABILITY_FAILED', '''Data is reliabile.''')
RELIABILITY_OK = 1 # Data is not reliabile.
enums['RELIABILITY'][1] = EnumEntry('RELIABILITY_OK', '''Data is not reliabile.''')
RELIABILITY_ENUM_END = 2 # 
enums['RELIABILITY'][2] = EnumEntry('RELIABILITY_ENUM_END', '''''')

# message IDs
MAVLINK_MSG_ID_BAD_DATA = -1
MAVLINK_MSG_ID_SNS = 21

class MAVLink_sns_message(MAVLink_message):
        '''
        The satellite navigation system.
        '''
        id = MAVLINK_MSG_ID_SNS
        name = 'SNS'
        fieldnames = ['time_boot_ms', 'reliability', 'time_sns', 'latitude', 'longitude', 'altitude', 'groundspeed', 'climb', 'course']
        ordered_fieldnames = ['time_sns', 'time_boot_ms', 'latitude', 'longitude', 'altitude', 'groundspeed', 'climb', 'course', 'reliability']
        fieldtypes = ['uint32_t', 'uint8_t', 'uint64_t', 'int32_t', 'int32_t', 'int32_t', 'float', 'float', 'float']
        format = '<QIiiifffB'
        native_format = bytearray('<QIiiifffB', 'ascii')
        orders = [1, 8, 0, 2, 3, 4, 5, 6, 7]
        lengths = [1, 1, 1, 1, 1, 1, 1, 1, 1]
        array_lengths = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        crc_extra = 25
        unpacker = struct.Struct('<QIiiifffB')

        def __init__(self, time_boot_ms, reliability, time_sns, latitude, longitude, altitude, groundspeed, climb, course):
                MAVLink_message.__init__(self, MAVLink_sns_message.id, MAVLink_sns_message.name)
                self._fieldnames = MAVLink_sns_message.fieldnames
        ...
        def pack(self, mav, force_mavlink1=False):
                return MAVLink_message.pack(self, mav, 25, struct.pack('<QIiiifffB', self.time_sns, self.time_boot_ms, self.latitude, self.longitude, self.altitude, self.groundspeed, self.climb, self.course, self.reliability), force_mavlink1=force_mavlink1)


mavlink_map = {
        MAVLINK_MSG_ID_SNS : MAVLink_sns_message,
}
...
class MAVLink(object):
        ...
        def decode(self, msgbuf):
                ...
                return m
        def sns_encode(self, time_boot_ms, reliability, time_sns, latitude, longitude, altitude, groundspeed, climb, course):
                '''
                The satellite navigation system.

                time_boot_ms              : Timestamp (time since system boot) [ms] (type:uint32_t)
                reliability               : Reliability (type:uint8_t, values:RELIABILITY)
                time_sns                  : Satellite timestamp [ms] (type:uint64_t)
                latitude                  : Latitude (WGS84, EGM96 ellipsoid) [degE7] (type:int32_t)
                longitude                 : Longitude (WGS84, EGM96 ellipsoid) [degE7] (type:int32_t)
                altitude                  : Satellite AMSL altitude [mm] (type:int32_t)
                groundspeed               : Current ground speed [m/s] (type:float)
                climb                     : Satellite climb [m/s] (type:float)
                course                    : Course over ground (NOT heading, but direction of movement) in radians (0..+2pi) [rad] (type:float)

                '''
                return MAVLink_sns_message(time_boot_ms, reliability, time_sns, latitude, longitude, altitude, groundspeed, climb, course)

        def sns_send(self, time_boot_ms, reliability, time_sns, latitude, longitude, altitude, groundspeed, climb, course, force_mavlink1=False):
                '''
                The satellite navigation system.
                ...
                '''
                return self.send(self.sns_encode(time_boot_ms, reliability, time_sns, latitude, longitude, altitude, groundspeed, climb, course), force_mavlink1=force_mavlink1)
