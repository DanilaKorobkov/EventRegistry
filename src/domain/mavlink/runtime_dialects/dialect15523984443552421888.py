...
DIALECT = 'dialect15523984443552421888'
...
class MAVLink_attitude_message(MAVLink_message):
        ...
        name = 'ATTITUDE'
        fieldnames = ['time_boot_ms', 'reliability', 'roll', 'pitch', 'yaw']
        ordered_fieldnames = ['time_boot_ms', 'roll', 'pitch', 'yaw', 'reliability']
        fieldtypes = ['uint32_t', 'uint8_t', 'float', 'float', 'float']
        format = '<IfffB'
        native_format = bytearray('<IfffB', 'ascii')
        orders = [0, 4, 1, 2, 3]
        lengths = [1, 1, 1, 1, 1]
        array_lengths = [0, 0, 0, 0, 0]
        crc_extra = 172
        unpacker = struct.Struct('<IfffB')

        def __init__(self, time_boot_ms, reliability, roll, pitch, yaw):
        ...
        def pack(self, mav, force_mavlink1=False):
                return MAVLink_message.pack(self, mav, 172, struct.pack('<IfffB', self.time_boot_ms, self.roll, self.pitch, self.yaw, self.reliability), force_mavlink1=force_mavlink1)
...
class MAVLink(object):
        ...
        def decode(self, msgbuf):
                ...
                return m
        def attitude_encode(self, time_boot_ms, reliability, roll, pitch, yaw):
                '''
                ...
                roll                      : Roll angle in radians (-pi..+pi) [rad] (type:float)
                pitch                     : Pitch angle in radians (-pi..+pi) [rad] (type:float)
                yaw                       : Yaw angle in radians (-pi..+pi) [rad] (type:float)

                '''
                return MAVLink_attitude_message(time_boot_ms, reliability, roll, pitch, yaw)

        def attitude_send(self, time_boot_ms, reliability, roll, pitch, yaw, force_mavlink1=False):
                '''
                The attitude in the aeronautical frame (right-handed, Z-down, X-front,
                Y-right).
                ...
                '''
                return self.send(self.attitude_encode(time_boot_ms, reliability, roll, pitch, yaw), force_mavlink1=force_mavlink1)
